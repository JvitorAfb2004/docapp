...
export default function CriarDocumentoUnificadoPage() {
  ...
  const dfdSections = [
    {
      title: 'Dados Comuns',
      description: 'Informações compartilhadas entre DFD e ETP',
      fields: [
        { name: 'numeroSGD', label: 'Número do SGD', type: 'text', required: true },
        { name: 'descricaoNecessidade', label: 'Descrição da Necessidade', type: 'textarea', required: true },
        ...
        { name: 'protocoloPNCP', label: 'Protocolo PNCP', type: 'text', placeholder: 'Ex: PNCP-2025-001' },
        { name: 'acaoOrcamentariaNumero', label: 'Número da Ação Orçamentária', type: 'text', placeholder: 'Ex: 2098' },
        { name: 'previsaoPCA', label: 'Demanda está no PCA?', type: 'checkbox', required: true },
        { name: 'justificativaPCA', label: 'Justificativa (se não estiver no PCA)', type: 'textarea' }
      ]
    },
    {
      title: 'Identificação do DFD',
      description: 'Dados específicos do Documento de Formalização de Demanda',
      fields: [
        { name: 'numeroDFD', label: 'Número do DFD', type: 'text', required: true },
        { name: 'especificacaoBensServicos', label: 'Especificação Detalhada', type: 'textarea', required: true },
        { name: 'codigoSIGA', label: 'Código SIGA', type: 'text' },
        { name: 'recursoConvenio', label: 'Recurso é de Convênio?', type: 'checkbox' }
      ]
    },
    {
      title: 'Especificações Detalhadas',
      description: 'Detalhamento dos bens/serviços com quantidades',
      fields: [
        { name: 'item', label: 'Item', type: 'text', required: true },
        { name: 'quantidade', label: 'Quantidade', type: 'text', required: true },
        { name: 'unidade', label: 'Unidade', type: 'text', required: true },
        { name: 'especificacaoDetalhada', label: 'Especificação Detalhada', type: 'textarea', required: true }
      ]
    },
    {
      title: 'Fiscais e Gestores',
      description: 'Indicação de fiscais e gestores responsáveis',
      fields: [
        { name: 'fiscalTitular', label: 'Fiscal Titular', type: 'text', required: true },
        { name: 'fiscalSuplente', label: 'Fiscal Suplente', type: 'text' },
        { name: 'gestorTitular', label: 'Gestor Titular', type: 'text', required: true },
        { name: 'gestorSuplente', label: 'Gestor Suplente', type: 'text' }
      ]
    },
    {
      title: 'Demandante',
      description: 'Informações do demandante da contratação',
      fields: [
        { name: 'demandante.orgao', label: 'Órgão do Demandante', type: 'text', required: true },
        { name: 'demandante.setor', label: 'Setor do Demandante', type: 'text', required: true },
        { name: 'demandante.cargo', label: 'Cargo/Função do Demandante', type: 'text', required: true },
        { name: 'demandante.nome', label: 'Nome do Demandante', type: 'text', required: true },
        { name: 'demandante.numeroFuncional', label: 'Número Funcional do Demandante', type: 'text', required: true }
      ]
    },
    {
      title: 'Responsáveis',
      description: 'Responsáveis pela ação orçamentária e planejamento',
      fields: [
        { name: 'responsaveisAcaoOrcamentaria', label: 'Responsáveis pela Ação Orçamentária', type: 'dynamic_array', required: true },
        { name: 'responsavelPlanejamento.nome', label: 'Nome do Responsável pelo Planejamento', type: 'text', required: true },
        { name: 'responsavelPlanejamento.numeroFuncional', label: 'Número Funcional do Responsável pelo Planejamento', type: 'text', required: true },
        { name: 'responsavelPlanejamento.cargo', label: 'Cargo/Função do Responsável pelo Planejamento', type: 'text', required: true }
      ]
    }
  ];
  ...
  const etpSections = [
    {
      title: 'Identificação do ETP',
      description: 'Dados específicos do Estudo Técnico Preliminar',
      fields: [
        ...
        { name: 'vigencia', label: 'Vigência', type: 'text', placeholder: 'Ex: 30 dias, 12 meses' },
        { name: 'prorrogavel', label: 'Prorrogável?', type: 'checkbox' },
        { name: 'servicoContinuado', label: 'É serviço continuado?', type: 'checkbox' },
        { name: 'justificativaServicoContinuado', label: 'Justificativa para serviço continuado', type: 'textarea' }
      ]
    },
    {
      ...
      fields: [
        ...
        { name: 'transicaoContratual', label: 'Necessita de transição contratual?', type: 'checkbox' },
        { name: 'normativosTecnicos', label: 'Normativos Técnicos Aplicáveis', type: 'textarea' },
    ...
    {
      title: 'Estimativas e Mercado',
      description: 'Estimativas de quantidades e levantamento de mercado',
      fields: [
        ...
        { name: 'serieHistorica.exercicio', label: 'Exercício da Série Histórica', type: 'text', placeholder: 'Ex: 2023' },
        { name: 'serieHistorica.quantidadeConsumida', label: 'Quantidade Consumida', type: 'text' },
        { name: 'serieHistorica.unidade', label: 'Unidade da Série Histórica', type: 'text' },
        { name: 'levantamentoMercado.fontes', label: 'Fontes de Pesquisa', type: 'textarea', placeholder: 'Ex: contratações similares, internet' },
        { name: 'levantamentoMercado.justificativa', label: 'Justificativa Técnica e Econômica', type: 'textarea' },
        { name: 'levantamentoMercado.restricoes', label: 'Restrições de Fornecedores', type: 'textarea' },
        { name: 'levantamentoMercado.tratamentoME', label: 'Aplicar tratamento diferenciado para ME/EPP?', type: 'checkbox' },
        { name: 'meiosPesquisaPrecos', label: 'Meios Usados na Pesquisa de Preços', type: 'textarea' }
      ]
    },
    {
      title: 'Quantitativos Detalhados',
      description: 'Detalhamento dos quantitativos por item',
      fields: [
        { name: 'quantitativos.item', label: 'Item do Quantitativo', type: 'text' },
        { name: 'quantitativos.descricao', label: 'Descrição do Item', type: 'textarea' },
        { name: 'quantitativos.unidade', label: 'Unidade', type: 'text' },
        { name: 'quantitativos.quantidade', label: 'Quantidade', type: 'text' }
      ]
    },
    {
      title: 'Solução e Requisitos',
      description: 'Descrição da solução e requisitos técnicos',
      fields: [
        { name: 'descricaoSolucao', label: 'O que será contratado', type: 'textarea' },
    ...
    },
    {
      ...
      description: 'Resultados pretendidos e impactos ambientais',
      fields: [
        ...
        { name: 'resultadosPretendidos.notaExplicativa', label: 'Nota Explicativa', type: 'textarea' },
        { name: 'providenciasPrevias.providencias', label: 'Providências Pendentes', type: 'textarea' },
        { name: 'providenciasPrevias.requisitosGestao', label: 'Requisitos para Gestão', type: 'textarea' },
        { name: 'providenciasPrevias.requisitosFiscalizacao', label: 'Requisitos para Fiscalização Contratual', type: 'textarea' },
        { name: 'contratacoesCorrelatas', label: 'Existem contratações correlatas?', type: 'checkbox' },
        { name: 'indicacaoContratacoesCorrelatas', label: 'Indicação das contratações correlatas', type: 'textarea' },
    ...
    },
    {
      ...
      description: 'Viabilidade e conclusão do estudo',
      fields: [
        ...
        { name: 'posicionamentoConclusivo.textoConclusivo', label: 'Texto Conclusivo', type: 'textarea', required: true }
      ]
    }
  ];
  ...
  const generateDocumentsWithLoading = async () => {
    ...
    try {
      ...
      const progressSteps = [
        { progress: 10, message: 'Processando dados do formulário...', delay: 500 },
        { progress: 25, message: 'Enviando dados para a IA...', delay: 2000 },
        { progress: 50, message: 'IA processando informações...', delay: 8000 },
        { progress: 75, message: 'Gerando documento DFD...', delay: 3000 },
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white rounded-lg p-6 max-w-md w-full mx-4">
              <div className="text-center mb-4">
                ...
                <p className="text-gray-600">
                  Seus documentos foram gerados com sucesso e estão prontos para download.
                </p>
              </div>
